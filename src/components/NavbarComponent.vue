...
<template>
    <header class="fixed  inset-x-0 top-0 z-50 elevation-2">
        <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
            <div class="flex lg:flex-1">
                <a href="/" class="min-w-40">
                    <span class="sr-only">S.N.C Company</span>
                    <img src="@/assets/logo.png" width="80" alt="Company Logo" />
                </a>
            </div>
            ...
            <div class="hidden lg:flex lg:gap-x-12">
                <a v-for="item in navigation" :key="item.name" :href="item.href"
                    class="text-sm font-semibold leading-6 text-gray-900 hover:#314351" :style="color=primary" >
                    {{ item . name }}
                </a>
            </div>
